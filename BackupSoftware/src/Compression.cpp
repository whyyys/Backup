 ...
 */
Compression::Compression(std::string file_path_){
...
void Compression::Encode(HuffNode *root, std::string code)
{
    root->code = code;
    // 叶子节点
...
void Compression::CleanHuffmanTree(HuffNode *root)
{
    if (!root)
        return;
    if (root->left)
