...
Encryption::Encryption(std::string file_path_, std::string password_)
{
...
bool Encryption::encrypt() {
    ...
    // 加密后的文件
    fs::path ept_path(file_path);
    ept_path += FILE_SUFFIX_ENCRYPT;
    FileInfo file_out(ept_path);
...
int Encryption::decrypt() {
    ...
    // 解密后的文件
    fs::path dpt_path(file_path);
    dpt_path.replace_extension("");
    FileInfo file_out(dpt_path);
    if (!file_out.OpenFile(std::ios::out | std::ios::binary | std::ios::trunc))
        return -2;
    file_out.WriteBackupInfo(info);
